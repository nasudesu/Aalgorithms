...
class ReservedMemory():
    ...
    def copy(self, mem_source: ReservedMemory, count: int = None, source_index: int = 0,
             destination_index: int = 0) -> None:
        """
        ...
        self._reserved_memory[destination_index:destination_index + count] = mem_source._reserved_memory[
...
class IntArray():
    ...
    def remove(self, index: int) -> int:
        popperList = []
        indexNum = self.__getitem__(index)
        if -1 < index < self._size + 1:
            for _ in range(self._size - index):
                value = self.pop()
                if value != indexNum: popperList.append(value)
            popperList.reverse()
            for j in popperList: self.append(j)
            return indexNum
        else:
            raise IndexError
